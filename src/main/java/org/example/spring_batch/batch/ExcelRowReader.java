...
public class ExcelRowReader implements ItemStreamReader<Row> {
    ...
    private final String filePath;          // 파일의 경로
    private FileInputStream fileInputStream;// 파일을 열 객체
    private Workbook workbook;              // 엑셀 파일을 닫을 객체
    private Iterator<Row> rowCursor;        // 엑셀 파일 내부 시트에 각각의 행을 반복할 Iterator
    private int currentRowNumber;           // 어떤 행까지 진행했는지 행 번호를 기억할 값
    private final String CURRENT_ROW_KEY = "current.row.number";    // 배치 메타데이터 테이블에 어떤 행까지 수행했는지 적어놓기 위한 키 값
